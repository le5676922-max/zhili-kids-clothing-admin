    ...
    class AIAssistant {
        ...
        async sendMessage() {
            ...
            const displayContent = this.attachedImages.length > 0
                ...
                : content;
            this.addMessage('user', displayContent, this.attachedImages.map(f => ({ base64: f.base64, type: f.type })));
        ...
        addMessage(role, content, files = []) {
            ...
            if (files.length > 0) {
                filesHtml = '<div class="ai-message-images">' + files.map(f => {
                    ...
                    return `<div class="ai-message-file" onclick="AIAssistant.previewImage('${f.base64}')"><i class="bi bi-file-earmark-text"></i><span>${f.base64}</span></div>`;
                }).join('') + '</div>';
            }
